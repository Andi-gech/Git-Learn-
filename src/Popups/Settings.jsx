...
export default function Settings({ onClose }) {
  ...
  return (
    <div className="fixed z-50 top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white relative h-[500px] overflow-y-scroll rounded-md shadow-md shadow-zinc-600 p-4 w-[900px] flex">
        <div className="w-[25%] h-full bg-blue-500 flex flex-col justify-start sticky top-0">
          <div className="flex items-center justify-center h-[50px] bg-blue-400 rounded-md px-3 text-white">
            Organizations
          </div>
        </div>

        <div className="w-[75%] relative mb-[10px] bg-white px-6 ">
          {addDeduction && (
          ...
          <div className="h-[50px] w-full flex justify-between items-center mb-4">
            <p className="font-bold text-[18px]">Deductions</p>
            <div
              onClick={() => setAddDeduction(true)}
              className="w-[30px] h-[30px] bg-white shadow-md shadow-zinc-400 flex items-center justify-center cursor-pointer"
            >
          ...
          {/* Deduction List */}
          <div className="w-full max-h-[150px] overflow-y-auto rounded-md bg-zinc-50 p-2 flex flex-col space-y-2 mb-4">
            {data?.data.map((deduction) => (
              <div
                key={deduction.id}
                className="w-full bg-white shadow-sm rounded-md p-2 cursor-pointer hover:bg-gray-100"
              >
          ...
          <div className="w-full">
            <p className="font-bold mb-2">Organization Type</p>
